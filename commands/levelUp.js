...
const levelUpCommand = (username, socket, usersOnline, room, rooms, saveChatHistory) => {
  ...
  if (userToUpdate) {
    ...
    socket.emit('message', { user: 'Server', text: `The level of ${username} has been increased to ${userToUpdate.global.level}` });
  } else {
